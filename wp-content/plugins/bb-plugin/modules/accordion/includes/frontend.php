...
<div class="fl-accordion fl-accordion-<?php echo $settings->label_size; ?><?php echo ( $settings->collapse ) ? ' fl-accordion-collapse' : ''; ?>"<?php echo ( ! $settings->collapse ) ? ' multiselectable="true"' : ''; ?>>
	<?php
			...
			<div class="fl-accordion-item <?php echo $item_tab_class; ?>"<?php echo ( ! empty( $settings->id ) ) ? ' id="' . sanitize_html_class( $settings->id ) . '-' . $i . '"' : ''; ?>>
				<div class="fl-accordion-button" id="<?php echo 'fl-accordion-' . $module->node . '-tab-' . $i; ?>" aria-controls="<?php echo 'fl-accordion-' . $module->node . '-panel-' . $i; ?>" <?php echo $tabindex_attr; ?>>
					...
					<?php if ( 'right' === $settings->label_icon_position ) : ?>
						<a href="#" id="<?php echo $icon_id; ?>" class="fl-accordion-button-icon" tabindex="-1"><i class="fl-accordion-button-icon fl-accordion-button-icon-right <?php echo $item_tab_icon_class; ?>"><span class="sr-only"><?php echo ( $i > 0 || ! $settings->open_first ) ? 'Expand' : 'Collapse'; ?></span></i></a>
					<?php endif; ?>
